...
def set_reservation_setting():
    ...
    with get_db() as conn:
        ...
        conn.commit()
    return jsonify({'success': True, 'reservation_enabled': enabled == '1'})

@app.route('/api/settings/reservation', methods=['GET'])
def public_reservation_setting():
    """Students check this before submitting a reservation."""
    with get_db() as conn:
        row = conn.execute("SELECT value FROM settings WHERE key='reservation_enabled'").fetchone()
    enabled = (row['value'] == '1') if row else True
    return jsonify({'reservation_enabled': enabled})
...
def get_public_testimonials():
    with get_db() as conn:
        rows = conn.execute(
            'SELECT fullname, course, rating, message, created FROM testimonials WHERE is_approved=1 ORDER BY created DESC'
        ).fetchall()
...
def admin_get_testimonials():
    ...
    with get_db() as conn:
        rows = conn.execute(
            'SELECT id, fullname, course, rating, message, is_approved, created FROM testimonials ORDER BY created DESC'
        ).fetchall()
    return jsonify([dict(r) for r in rows])
